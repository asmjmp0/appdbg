...
public class ExprProcessor implements CodeConstants {
  ...
  private static final HashMap<Integer, Integer> functionMap = new HashMap<>();
  ...
  public void processBlock(BasicBlockStatement stat, PrimitiveExpressionList data, StructClass cl) {
    ...
    for (int i = 0; i < seq.length(); i++) {
      ...
      tempSet.add(offset);
      Set<Integer> offsets = offset >= 0 ? tempSet : null;
  ...
  public static List<ClassesProcessor.ClassNode> enclosingClassList() {
    ...
    List<ClassesProcessor.ClassNode> tempList = new LinkedList<>();
    tempList.add(enclosingClass);
    List<ClassesProcessor.ClassNode> enclosingClassList = new ArrayList<>(tempList);
